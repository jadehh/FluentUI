 ...
 */
class FluTreeNode final : public QObject {
    Q_OBJECT
...
public:
    ...
    Q_INVOKABLE bool hasNextNodeByIndex(const int index) {
        auto p = this;
    ...
    [[nodiscard]] Q_INVOKABLE bool checked() const {
        ...
        for (int i = 0; i <= _children.size() - 1; ++i) {
            if (const auto item = _children.at(i); !item->checked()) {
                return false;
            }
        }
        return true;
    ...
    Q_INVOKABLE bool hideLineFooter() {
        if (_parent) {
            const auto childIndex = _parent->_children.indexOf(this);
...
public:
    ...
    Q_INVOKABLE FluTreeNode *getNode(int row) const;

    Q_INVOKABLE void refreshNode(int row);

    Q_INVOKABLE void checkRow(int row, bool checked);

    Q_INVOKABLE bool hitHasChildrenExpanded(int row) const;
