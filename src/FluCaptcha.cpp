...
int generaNumber(const int number) {
...
void FluCaptcha::refresh() {
    this->_code.clear();
    for (int i = 0; i < 4; ++i) {
        if (const int num = generaNumber(3); num == 0) {
            this->_code += QString::number(generaNumber(10));
        } else if (num == 1) {
            constexpr int temp = 'A';
...
[[maybe_unused]] bool FluCaptcha::verify(const QString &code) const
{
    if (_ignoreCase) {
