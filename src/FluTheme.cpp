...
bool systemDark() {
    ...
    const QColor color = palette.color(QPalette::Window).rgb();
...
void FluTheme::checkUpdateDesktopImage() {
    ...
    QThreadPool::globalInstance()->start([=]() {
        _mutex.lock();
        if (const auto path = FluTools::getInstance()->getWallpaperFilePath(); _desktopImagePath != path) {
