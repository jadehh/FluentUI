...
void FluTools::setQuitOnLastWindowClosed(const bool val) {
    QGuiApplication::setQuitOnLastWindowClosed(val);
}

void FluTools::setOverrideCursor(const Qt::CursorShape shape) {
...
QColor FluTools::withOpacity(const QColor &color, qreal opacity) {
    const int alpha = qRound(opacity * 255) & 0xff;
...
int FluTools::cursorScreenIndex() {
    int screenIndex = 0;
    if (const int screenCount = QGuiApplication::screens().count(); screenCount > 1) {
...
#if defined(Q_OS_WIN)
    const QSettings regKey{
        QString::fromUtf8(R"(HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion)"),
        QSettings::NativeFormat};
    if (regKey.contains(QString::fromUtf8("CurrentBuildNumber"))) {
        const auto buildNumber = regKey.value(QString::fromUtf8("CurrentBuildNumber")).toInt();
        return buildNumber;
    }
#endif
    return -1;
...
bool FluTools::isWindows11OrGreater() {
    ...
    if (var.isNull()) {
#if defined(Q_OS_WIN)
        if (const auto buildNumber = windowBuildNumber(); buildNumber >= 22000) {
...
bool FluTools::isWindows10OrGreater() {
    ...
    if (var.isNull()) {
#if defined(Q_OS_WIN)
        if (const auto buildNumber = windowBuildNumber(); buildNumber >= 10240) {
            var = QVariant::fromValue(true);
            return true;
        }
#endif
...
QColor FluTools::imageMainColor(const QImage &image, const double bright) {
    ...
    return QColor(static_cast<int>(bright * r / t) > 255 ? 255 : static_cast<int>(bright * r / t),
                  static_cast<int>(bright * g / t) > 255 ? 255 : static_cast<int>(bright * g / t),
                  static_cast<int>(bright * b / t) > 255 ? 255 : static_cast<int>(bright * b / t));
}
