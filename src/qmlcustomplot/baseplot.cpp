...
void BasePlot::onChartViewSizeChanged()
{
    ...
    m_customPlot->setViewport(QRect(0, 0, static_cast<int>(width()), static_cast<int>(height())));
    m_customPlot->axisRect()->setOuterRect(QRect(0, 0, static_cast<int>(width()), static_cast<int>(height())));
    m_customPlot->axisRect()->setMinimumMargins (QMargins(0, 0, 0, 0));
    m_customPlot->axisRect()->setMargins(QMargins(0, 0, 0, 0));
}

void BasePlot::routeMouseEvents(const QMouseEvent *event) const
{
    QMouseEvent* newEvent = new QMouseEvent(event->type(), event->localPos(), event->button(), event->buttons(), event->modifiers());
    QCoreApplication::postEvent(m_customPlot, newEvent);
}

void BasePlot::routeWheelEvents(const QWheelEvent *event) const
{
