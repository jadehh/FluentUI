...
Graph::Graph(QCPGraph* graph, QCustomPlot *parentPlot, QObject *parent)
    : QObject(parent), m_parentPlot(parentPlot), m_graph(graph)
{
...
void Graph::setData(const QVector<double> &keys, const QVector<double> &values) const noexcept
{
    m_graph->setData(keys, values);
    m_parentPlot->replot();
}

void Graph::addData(const double key, const double value) const noexcept
{
    m_graph->addData(key, value);
    m_parentPlot->replot();
}

void Graph::removeDataBefore(const double key) const noexcept
{
    m_graph->data()->removeBefore(key);
    m_parentPlot->replot();
}

void Graph::clearData() const noexcept
{
    m_graph->data()->clear();
    m_parentPlot->replot();
}

void Graph::set_visible(const bool value) noexcept
{
...
void Graph::set_antialiased(const bool value) noexcept
{
...
void Graph::set_graphWidth(const int value) noexcept
{
